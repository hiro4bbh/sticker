...
func TestJaccardHashing(t *testing.T) {
	...
	hashing.Add(x1, 0)
	hashing.Add(x2, 1)
	goassert.New(t, map[uint32]uint32{0: 16, 1: 8}).Equal(hashing.FindNears(x1).Map())
	goassert.New(t, map[uint32]uint32{0: 8, 1: 16}).Equal(hashing.FindNears(x2).Map())
	goassert.New(t, []int{1, 2, 1, 1, 2, 2, 1, 1, 2, 1, 2, 2, 1, 1, 2, 2}, []int{16, 8, 0, 0, 0, 0, 0, 0}).Equal(hashing.Summary())
	...
	goassert.New(t, []int{1, 2, 1, 1, 2, 2, 1, 1, 2, 1, 2, 2, 1, 1, 2, 2}, []int{8, 0, 0, 0, 0, 0, 0, 16}).Equal(hashing.Summary())
